...
/*@-null*/
static int codeDefault[] = {310, 315};
static int edgeDefault[] = {1, 1};
static char* commandDefault[] = {"/sbin/shutdown -h now", "/sbin/shutdown -h now"};
...
static Cmdline cmd = {
  ...
  /* deviceP = */ 1,
  /* device = */ "Padix Co. Ltd. USB, 2-axis, 14-button gamepad",
  /* deviceC = */ 1,
...
usage(void)
{
  fprintf(stderr,"%s","   [-c code] [-e edge] [-cmd command] [-fg] [-d] [-q] [-dev device]\n");
  fprintf(stderr,"%s","      Shut down System when button is pressed/released\n");
  fprintf(stderr,"%s","      -c: scan codes to react to\n");
  fprintf(stderr,"%s","          1 or more int values\n");
  fprintf(stderr,"%s","          default: `310' `315'\n");
  fprintf(stderr,"%s","      -e: run command on negative(0) oder positive(1) edge\n");
  fprintf(stderr,"%s","          1 or more int values between 0 and 1\n");
  fprintf(stderr,"%s","          default: `1' `1'\n");
  fprintf(stderr,"%s","    -cmd: command to execute\n");
  fprintf(stderr,"%s","          1 or more char* values\n");
  fprintf(stderr,"%s","          default: `/sbin/shutdown -h now' `/sbin/shutdown -h now'\n");
  fprintf(stderr,"%s","     -fg: run in foreground do not daemonize\n");
  fprintf(stderr,"%s","      -d: run in debug mode (implies foreground)\n");
  fprintf(stderr,"%s","      -q: query available devices\n");
  fprintf(stderr,"%s","    -dev: device to use for input\n");
  fprintf(stderr,"%s","          1 char* value\n");
  fprintf(stderr,"%s","          default: `Padix Co. Ltd. USB, 2-axis, 14-button gamepad'\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    if( 0==strcmp("-c", argv[i]) ) {
      int keep = i;
      cmd.codeP = 1;
      i = getIntOpts(argc, argv, i, &cmd.code, 1, -1);
      cmd.codeC = i-keep;
      continue;
    }
    ...
    if( 0==strcmp("-e", argv[i]) ) {
      int keep = i;
      cmd.edgeP = 1;
      i = getIntOpts(argc, argv, i, &cmd.edge, 1, -1);
      cmd.edgeC = i-keep;
      checkIntLower("-e", cmd.edge, cmd.edgeC, 1);
      checkIntHigher("-e", cmd.edge, cmd.edgeC, 0);
      continue;
    }
    ...
    if( 0==strcmp("-cmd", argv[i]) ) {
      int keep = i;
      cmd.commandP = 1;
      i = getStringOpts(argc, argv, i, &cmd.command, 1, -1);
      cmd.commandC = i-keep;
      continue;
    }

    if( 0==strcmp("-fg", argv[i]) ) {
      cmd.foregroundP = 1;
      continue;
    }
    ...
    if( 0==strcmp("-dev", argv[i]) ) {
      int keep = i;
      cmd.deviceP = 1;
      i = getStringOpt(argc, argv, i, &cmd.device, 1);
      cmd.deviceC = i-keep;
      continue;
    }
